...
My work makes several contributions relative to this literature.

Prior work has established that peer review reliability is low
...
The fundamental challenge is that /true quality is unobserved/. Empirical data
can reveal what was accepted and rejected, but not what /should/ have been
accepted and rejected. Without knowing true quality, we cannot calculate false
positive or false negative rates---only acceptance rates. Moreover, journals do
not randomly assign reviewers, vary the number of reviewers experimentally, or
publish counterfactual decisions. This makes causal identification of peer
review mechanics essentially impossible with observational data.

Simulation addresses this limitation by generating papers with /known/ true
quality, subjecting them to modeled review processes, and comparing decisions
to ground truth. This allows direct measurement of error rates under controlled
conditions. The cost is that results depend on modeling assumptions; the
...
(especially noise) to empirical reliability data, grounding the simulation in
observed reviewer behavior while enabling analysis that observational data
cannot support.
...
correlation ($\rho_{dim} = 0.4$) suggests that baseline FNR would fall from
approximately 45% to approximately 38%---still substantial, but attenuated. My
...
The single-reviewer case (n = 1) deserves particular attention because JAR and
JAE frequently use only one reviewer. With a single reviewer, FNR rises to
52%---meaning that /more than half/ of deserving papers are rejected based on
one person's noisy assessment. The 6.9 percentage point improvement from adding
a second reviewer represents the largest marginal gain in the table, yet these
journals have chosen speed over accuracy. This institutional choice has
consequences: at identical true quality, a paper's fate at a single-reviewer
journal is substantially more random than at a two-reviewer journal.
...
Noise does not change true quality---it changes /observed/ quality. When
reviewers add noise to their assessments, some excellent papers are observed as
merely good (and rejected), while some mediocre papers are observed as
acceptable (and published). The threshold acts as an attractor: with sufficient
noise, acceptance rates across quality levels converge toward whatever baseline
rate exists at the threshold.

Consider the compression in acceptance rates under high noise:
- Quality 90 → 53% acceptance (down from 94% with low noise)
- Quality 50 → 7% acceptance (up from 0.1% with low noise)

The ratio of acceptance rates shrinks from 940:1 to merely 8:1. In other words,
high noise dramatically weakens the correlation between true quality and
publication. The published literature becomes less representative of actual
quality differences and more representative of luck.

An observer examining only published papers would see a narrower quality
distribution than actually exists in submissions---the tails are truncated from
both ends. Excellent papers that deserved publication are missing (rejected due
to unlucky reviewer draws), while mediocre papers that should have been
filtered out are present (accepted due to lucky draws). The published record
thus /systematically underrepresents/ the true variance in research quality.

This convergence to mediocrity may partly explain why top journals are
perceived as publishing increasingly similar work. If noise compresses the
quality distribution of accepted papers, and if reviewers from similar
methodological backgrounds add correlated noise (see Section [[#robustness]]),
then the published literature will trend toward a narrow band of "safe"
research that reliably clears the threshold without attempting
excellence---exactly the pattern documented by
cite:burtonPerceptionsAccountingAcademics2024.

Importantly, this mechanism operates independently of editorial intent or
reviewer competence. Even a journal staffed entirely by well-meaning,
highly-qualified reviewers will exhibit convergence to mediocrity if the
review process is sufficiently noisy. The problem is structural, not
personnel.

** Quality vs. Volume: Incentive Implications

The noisy structure of peer review creates incentive structures that may favor
quantity over quality. To explore this implication, I compare expected
publications under four hypothetical strategies for a researcher with fixed
total effort (at threshold = 70, 2 reviewers, noise SD = 30):
...
Under these parameters, the "two very good" strategy yields 46% higher expected
publications than "one excellent," despite producing lower-quality work. This
occurs because stochastic acceptance rewards multiple attempts: even if each
...
Given that noise helps below-threshold papers, a natural question arises: could
a researcher maximize expected publications by producing /many/ mediocre papers,
treating each as a lottery ticket? I extend the strategy comparison to include
...
|------------------+--------+---------+-----------+---------+---------------|
| Two very good    |      2 |      80 |     45.5% |    0.91 | Above         |
| Two good         |      2 |      75 |     35.0% |    0.70 | Above         |
| One excellent    |      1 |      90 |     69.8% |    0.70 | Above         |
| Four decent      |      4 |      65 |     15.8% |    0.63 | Below         |
| Four mediocre    |      4 |      55 |      5.0% |    0.20 | Below         |
| Eight poor       |      8 |      45 |      1.2% |    0.10 | Below         |
| Sixteen lottery  |     16 |      35 |      0.2% |    0.03 | Below         |

The results reveal a non-linear relationship. The "lottery ticket" strategy
fails: acceptance probability drops faster than paper count increases. Sixteen
papers at quality 35 yield only 0.03 expected publications, far below one
excellent paper (0.70).

However, the results also reveal that /slightly/ below-threshold papers can
compete with excellent ones. Four papers at quality 65 (five points below
threshold) yield 0.63 expected publications---only 10% less than one excellent
paper at quality 90. This suggests a "near-miss" region where the lottery
effect is meaningful but not dominant.

The optimal strategy remains firmly above threshold: "two very good" papers at
quality 80 dominate all alternatives by a wide margin (0.91 expected
publications). The key insight is that researchers should aim for the /lowest
quality that reliably clears the threshold/, not the highest quality possible.
Excess quality above threshold provides diminishing returns, while quality
below threshold wastes effort on long-shot lottery tickets.

*** Implications for Researcher Behavior

If researchers respond rationally to these incentives, this would help explain
several observed phenomena in accounting research:
...
co-authorship, or multi-period optimization. The strategy comparison should
therefore be interpreted as /comparative statics/---showing how expected
publications vary with quality and quantity choices---rather than as
predictions about actual researcher behavior. A full equilibrium model would
require additional assumptions about effort costs and researcher heterogeneity
that are beyond this paper's scope.

That said, the incentive gradients I document are consistent with observed
behavior and with cite:azoulayDoesPeerReview2025, who provide evidence that
peer review penalizes researchers who pursue bold, unconventional work. If
innovative research faces higher variance in reviewer assessments (less
consensus on what "good" looks like), the AND-gate effect would be amplified,
making conservative, incremental work relatively more attractive in expected
value terms.
...
pool merely confirms the shared bias.

However, the empirical evidence from accounting suggests that reviewer
...
reviewers agree to accept or reject, and the process concludes rapidly.
Instead, we observe the opposite. Accounting journals have longer times to
publication than comparable journals in other fields despite using fewer
reviewers (cite:woodComparingPublicationProcess2016). Papers routinely go
through multiple revision rounds as authors attempt to satisfy divergent
reviewer demands. Management Science found it necessary to impose an explicit
two-round limit to prevent excessive iterations---a policy that would be
unnecessary if reviewers typically agreed. Papers rejected at one journal are
often accepted at another with different reviewers, suggesting the original
rejection reflected reviewer-specific views rather than consensus about
quality.

This pattern is consistent with my baseline assumption of independent errors.
The low ICC reported by cite:bornmannReliabilityGeneralizationStudyJournal2010
is not measurement error in the traditional sense---noise that could be reduced
with better instruments. Rather, it reflects genuine lack of consensus about
what constitutes quality. Reviewers hold fundamentally different views about
what makes a paper good, leading to the contradictory demands and extended
review processes that characterize the field.

The correlated-errors analysis above should therefore be interpreted as a
/counterfactual/: what would happen if reviewers did agree? The answer is that
adding reviewers would provide little benefit---but also that decisions would
be faster and more consistent. The fact that we observe slow, inconsistent
decisions suggests the independent-errors baseline is the empirically relevant
case. My main results, which assume independent errors, are thus grounded in
...
To provide a sense of plausible bounds, I summarize results under optimistic
...
Several implications above may seem counterintuitive, so it is worth
summarizing why common-sense approaches to improving peer review can backfire.

/More detailed rubrics do not help./ The intuition that clearer criteria should
improve reliability fails because each additional criterion is another
dimension on which noise can cause rejection. AND-gating turns comprehensive
evaluation into a liability.

/Adding reviewers has sharply diminishing returns./ The intuition that more
opinions should converge on truth fails because averaging only reduces variance
by $1/\sqrt{n}$. Going from 2 to 10 reviewers improves accuracy by only about 9
percentage points---often not worth the cost.

/Reviewer training does not improve reliability./ The intuition that better
training should produce better reviews fails because low reliability reflects
fundamental uncertainty about quality, not reviewer incompetence
(cite:hesselbergReviewerTrainingImproving2023).

/Transparency does not automatically improve outcomes./ The intuition that open
review or published referee reports should discipline reviewers fails because
the problem is noise, not effort. Reviewers who try harder still disagree.
...
capture. This feedback loop could be self-reinforcing: if noisy review induces
quantity-over-quality strategies, the resulting submissions become more
homogeneous in quality (lower $\tau$), which my sensitivity analysis suggests
would /increase/ false rejection rates, which would further strengthen
incentives for quantity over quality. The system could thus converge to a
low-quality, high-noise equilibrium even if it began with higher-quality
submissions. Modeling this dynamic would require combining my simulation
framework with an equilibrium model of researcher behavior.
