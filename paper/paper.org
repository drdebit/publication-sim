...
reviews prior research on peer review reliability and positions our
contribution. Section [[#model]] develops a formal model of peer review as
noisy signal extraction. Section [[#simulation]] describes our simulation
...
effects. Section [[#discussion]] discusses implications for accounting
journals and possible reforms. Section [[#conclusion]] concludes.
...
The empirical foundation for our simulation comes from a large literature
...
the AND-gate effect we model below.
...
Our work builds on a growing literature using simulation to study peer
...
a precursor to the strategic behavior we analyze.
...
odds of acceptance, remarkably consistent with our false negative rates
...
rates. Our work extends this insight by formally modeling the AND-gate
...
essential for calibrating our simulation and interpreting its implications.
...
our analysis. With one reviewer, there is no averaging of assessments to
...
opportunity for careful deliberation. The simulation results we present
...
particularly vulnerable to the measurement error problems we document.
...
are practical limits to this strategy. Our simulation quantifies these
diminishing returns and shows that meaningful improvements require
structural changes rather than marginal additions.

** Contributions

Our work makes several contributions relative to this literature. First, we
...
strategic researcher behavior in a single model. Second, we calibrate to
empirical data rather than using arbitrary parameters, allowing quantitative
predictions about error rates. Third, we derive the researcher's optimal
...
Fourth, we examine interaction effects (reviewers × noise, reviewers ×
dimensions, threshold × noise) that reveal when interventions are most
effective. Finally, we apply these insights specifically to accounting,
